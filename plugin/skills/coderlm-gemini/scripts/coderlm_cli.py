...
All state is cached in .gemini/coderlm_state/session.json relative to cwd.
...
def _resolve_instance() -> str | None:
    ...
    base = Path(".gemini/coderlm_state/instances")
    if base.exists():
        pid = os.getpid()
        while pid > 1:
            f = base / str(pid)
    ...
    hint = Path(".gemini/coderlm_state/active_instance")
    if hint.exists():
        try:
...
def _state_dir() -> Path:
    ...
    base = Path(".gemini/coderlm_state")
...
def cmd_init(args: argparse.Namespace) -> None:
    ...
    # Write PID-keyed instance files so CLI calls from this Gemini session
    ...
    # share a parent shell, they each find their own Gemini PID's file.
    instances_dir = Path(".gemini/coderlm_state/instances")
    ...
    active_file = Path(".gemini/coderlm_state/active_instance")
...
def _load_agent_system_prompt(agent_name: str) -> str:
    ...
    if not agent_file.exists():
        agent_file = Path(f".gemini/agents/{agent_name}.md")
    if not agent_file.exists():
...
def cmd_deep_query(args: argparse.Namespace) -> None:
    """Run full RLM exploration loop in a gemini sub-LM."""
    import shutil
    import subprocess
    ...
    if not gemini_bin:
        _output({"error": "'gemini' CLI not found on PATH"})
        return
    ...
    system_prompt = _load_agent_system_prompt("coderlm-deep-query-gemini")

    # Build user prompt with ABSOLUTE paths so sub-LM doesn't try to find them
    project_cwd = state.get("cwd", os.getcwd())
    cli_path = os.path.join(project_cwd, ".gemini/coderlm_state/coderlm_cli.py")
    
    # Prepend system prompt since gemini-cli doesn't have a separate flag
    full_prompt = f"{system_prompt}\n\nQuery: {args.query}\nCLI: {cli_path}\nCWD: {project_cwd}"

    # Spawn gemini with full REPL access
    env = {
        **os.environ,
        "CODERLM_SUBCALL": "1",
    ...
    # Read Final variable from server (sub-LM should have set it)
    try:
