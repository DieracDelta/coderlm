...
def _load_agent_system_prompt(agent_name: str = "coderlm-subcall-gemini") -> str:
    ...
    if not agent_file.exists():
        raise RuntimeError(
            ...
            f"Copy plugin/agents/{agent_name}.md to .gemini/agents/"
        )
...
def llm_query(prompt: str, context: str = "", chunk_id: str = "") -> dict:
    ...
    Supports recursive subcalls: the subagent can itself call llm_query()
    ...
    if not gemini_bin:
        raise RuntimeError("'gemini' CLI not found on PATH. Install Gemini CLI to use llm_query().")
    ...
    # Propagate env: CODERLM_SUBCALL=1 so cleanup is skipped in subcalls,
