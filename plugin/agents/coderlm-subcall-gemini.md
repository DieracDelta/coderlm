...
- One of:
  ...
  - A file path to read directly with the read_file tool
  - A chunk of code/text inline

Your job is to extract information relevant to the query from **only** the provided chunk.

## Procedure
1. If given a buffer name, read it: `python3 .gemini/coderlm_state/coderlm_cli.py buffer-peek BUFFER_NAME`
2. If given a file path, read it with the read_file tool
...
```bash
python3 .gemini/coderlm_state/coderlm_cli.py repl --full-output --code "
