---
name: coderlm-deep-query-gemini
description: Runs the full RLM Algorithm 1 exploration loop as a gemini sub-LM. Given a query, scouts the codebase via REPL metadata, delegates content analysis to subcall-batch/llm_query, and returns a structured Final result.
allowed-tools:
  - run_shell_command
...
The CLI path is **always** `.gemini/coderlm_state/coderlm_cli.py` (relative to the project root). If a `CLI:` line is provided in your input, use that exact path instead.

## BANNED ACTIONS — WILL CAUSE FAILURE

- **NEVER** use `find`, `ls`, `cat`, `head`, `tail`, `grep`, or `rg` via run_shell_command
- **NEVER** use the read_file tool on code files (.rs .py .ts .js .go .lean .nix .c .cpp .h)
- **NEVER** try to locate the CLI — the path is `.gemini/coderlm_state/coderlm_cli.py`
- **NEVER** check Python version or run setup commands — the CLI is ready to use

read_file is ONLY for config files (.json, .toml, .yaml, .md) if absolutely needed.
...
```bash
python3 .gemini/coderlm_state/coderlm_cli.py repl --code "
...
```bash
python3 .gemini/coderlm_state/coderlm_cli.py subcall-batch src/file.rs "What does this do?" --max-chunk-bytes 5000
```
...
```bash
python3 .gemini/coderlm_state/coderlm_cli.py deep-query "How does the permission middleware work?"
```
...
1. **NEVER use read_file on code files.** NEVER use find/cat/grep/ls via run_shell_command. ALL exploration through the CLI.
...
6. **If errors occur**, synthesize with what you have. Don't try workarounds with find/read_file.
